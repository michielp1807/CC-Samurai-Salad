...
local function newBuffer(x1, y1, x2, y2)
	...
	function buffer:setBufferSize(newX1, newY1, newX2, newY2)
		...
		if self.blittleWindow then
			self.blittleWindow = self.blittleWindow.reposition(self.x1, self.y1, self.x1 + self.width - 1,
	...
	---@param dXSub number?
	---@param dYSub number?
	function buffer:image(dx, dy, image, dXSub, dYSub)
		local screenBuffer = self.screenBuffer
		local width = self.width

		if self.blittleOn then
			local dx = (floor(dx + 0.5) - 1) * 2 + (dXSub or 0)
			local dy = (floor(dy + 0.5) - 1) * 3 + (dYSub or 0)
			local c2 = screenBuffer.c2
			...
			local dx = floor(dx + 0.5) - 1
			local dy = floor(dy + 0.5) - 1
			local c1 = screenBuffer.c1
			local c2 = screenBuffer.c2
			local chars = screenBuffer.chars
	...
	function buffer:drawBufferBLittle()
		...
		if not blittleWindow then
			self.blittleWindow = window.create(term.current(), self.x1, self.y1, self.x1 + self.width - 1,
				self.y1 + self.height - 1, false)
...
;
local function m(f, g, h, n, o, d)
	local p = o - n + 1;
	local q = d - o;
	for r = 0, p - 1 do g[r] = f[n + r] end
	;
	for r = 0, q - 1 do h[r] = f[o + 1 + r] end
	;
	local i = 0;
	local l = 0;
	local s = n;
	while i < p and l < q do
		if g[i][16] <= h[l][16] then
			f[s] = g[i]
			i = i + 1
		else
			f[s] = h[l]
...
;
local function timsort16(f)
	local c = #f;
	local t = b(a)
	local u = math.min;
	for i = 1, c, t do e(f, i, u(i + a - 1, c)) end
	;
	local v, w = {}, {}
	local x = t;
	while x <= c do
		for g = 1, c, 2 * x do
			local y = g + x - 1;
			local h = u(y + x, c)
			if y < h then m(f, v, w, g, y, h) end
		end
		;
		x = 2 * x
	end
	;
	for i = 1, math.floor(c / 2) do f[i], f[c - i + 1] = f[c - i + 1], f[i] end
end
...
local function newFrame(x1, y1, x2, y2)
	...
	function frame:drawObject(object, camera, cameraAngles)
		...
		local clippingEnabled = xCameraOffset * xCameraOffset + yCameraOffset * yCameraOffset +
			zCameraOffset * zCameraOffset < modelSize * modelSize * 4
		...
		for i = 1, #sortedPolygons do
			...
			local x1, y1, dX1 = map3dTo2d(polygon[1] + xCameraOffset, polygon[2] + yCameraOffset,
				polygon[3] + zCameraOffset)
			if dX1 > 0.00010000001 then
				local x2, y2, dX2 = map3dTo2d(polygon[4] + xCameraOffset, polygon[5] + yCameraOffset,
					polygon[6] + zCameraOffset)
				if dX2 > 0.00010000001 then
					local x3, y3, dX3 = map3dTo2d(polygon[7] + xCameraOffset, polygon[8] + yCameraOffset,
						polygon[9] + zCameraOffset)
					...
					elseif clippingEnabled then
						...
						local renderOffsetX, sXFactor, renderOffsetY, sYFactor = renderOffsetX, sXFactor, renderOffsetY,
							sYFactor
...
function models:plane(options)
	...
	---@class Model
	local plane = {
		newPoly(-1 * options.size, options.y, 1 * options.size, 1 * options.size, options.y, -1 * options.size,
			-1 * options.size, options.y, -1 * options.size, options.color),
		newPoly(-1 * options.size, options.y, 1 * options.size, 1 * options.size, options.y, 1 * options.size,
			1 * options.size, options.y, -1 * options.size, options.color),
	}
	return plane
